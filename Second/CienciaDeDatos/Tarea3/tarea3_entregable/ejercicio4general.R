...
##############################parametros del grid de busqueda
grid.lado <- 150
a <- .00001
b <- 7
c <- .0001
...
mu.s <- seq(a, b, length = grid.lado)#despues de 7 se muere
mu.s <- rep(mu.s, each = grid.lado)
sigma.s <- seq(c,d , length=grid.lado )#creo que mas de 70 se rompe
...
p1 <- ggplot(u, aes(x=train, y = test, colour = mu ,size = sigma  )) +
  ...
  ggtitle('Busqueda de parámetros de FLDA para el PIMA') +
...
p
############## visualizacion de los resultados con mejores parametros
y.hat.test <- FLDA(train = train, test = test, Y.train = y.train,  
                    Y.test = y.test, mu = .046 , 
                    sigma = 44.29, search = FALSE )
...
p2 <- ggplot(foo, aes(x=Comp.1, y = Comp.2, alpha=0.1,size = 0.5  )) +
  geom_point(aes(color = factor(foo$Y) )) +
  scale_color_manual(values =c('orange', 'purple')) +
  theme_minimal() +
  ggtitle('Clasificación inicial del dataset PIMA') +
...
p3 <- ggplot(foo, aes(x=Comp.1, y = Comp.2, alpha = 0.1, size=0.5 )) +
  ...
  scale_color_manual(values =c( 'orange', 'purple')) +
  theme_minimal() +
  ggtitle('Conjunto test PIMA clasificado por KLDA') +
  xlab('PC1')+ylab('PC2')+  theme(legend.position = 'none')  
p3
